...
export default function GalleryGrid({ images, showTitle = true, columns = 3, itemsPerPage = 12 }: GalleryGridProps) {
  ...
  const navigateImage = (direction: 'prev' | 'next') => {
    ...
    if (direction === 'prev') {
      setSelectedImageIndex(prev => prev === 0 ? paginatedImages.length - 1 : prev! - 1)
    } else {
      setSelectedImageIndex(prev => prev === paginatedImages.length - 1 ? 0 : prev! + 1)
    }
  }
  ...
  const handleTouchEnd = () => {
    ...
    else if (distance < -50) navigateImage('prev')
  }
        ...
        >
          ...
          {paginatedImages.length > 1 && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white text-sm bg-black/50 px-3 py-1 rounded-full">
              {selectedImageIndex + 1} / {paginatedImages.length}
            </div>
          )}

          <div className="relative max-w-5xl max-h-[85vh] w-full h-full flex items-center justify-center">
            <Image
              src={paginatedImages[selectedImageIndex].url}
