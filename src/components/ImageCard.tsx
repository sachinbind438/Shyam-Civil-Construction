...
export default function ImageCard({
  ...
  const Card = (
    <div
      className={`group relative  isolate cursor-pointer   transition-all duration-300 ${className} transform-gpu`}
    >
      {/* Image wrapper: keep relative for next/image fill */}
      <div className="relative w-full h-full overflow-hidden rounded-tr-[72px]  rounded-bl-[72px] z-0 transition-transform duration-300 group-hover:rounded-[72px]  transform-gpu">
        <Image
          src={src}
          alt={alt}
          fill
      ...
      {showOverlay && (
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-shadow duration-300 group-hover:rounded-[72px]" />
      )}
